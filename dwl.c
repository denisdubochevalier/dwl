...
void defaultgaps(const Arg *arg) { setgaps(gappx); }
...
void drawbar(Monitor *mon) {
  ...
  if (mon == selmon) {
    ...
    draw_text(pix, mon->b.width - tw, 0, tw, mon->b.height, 0, stext,
              &normbarfg);
  }
  ...
  x = draw_text(pix, x, 0, w, mon->b.height, lrpad / 2, mon->ltsymbol,
                &normbarfg);
...
void incgaps(const Arg *arg) { setgaps(selmon->gappx + arg->i); }
